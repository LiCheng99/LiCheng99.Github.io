...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const coverColor = function () {
    ...
    if (path !== undefined) {
      ...
      httpRequest.onreadystatechange = function () {
        if (httpRequest.readyState == 4 && httpRequest.status == 200) {
          ...
          try {
            ...
            // 设置转化后的值
            root.style.setProperty("--licheng-bar-background", value);
            // 修改顶栏tab bar状态栏
            anzhiyu.initThemeColor();
          } catch (err) {
            // 在这里处理 JSON.parse() 抛出的错误
            root.style.setProperty("--licheng-bar-background", "var(--licheng-main)");
            // 修改顶栏tab bar状态栏
            anzhiyu.initThemeColor();
          }
        }
      ...
      root.style.setProperty("--licheng-bar-background", "var(--licheng-meta-theme-color)");
    }
    anzhiyu.initThemeColor();
  };
